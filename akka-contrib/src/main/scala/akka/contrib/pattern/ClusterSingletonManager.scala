...
object ClusterSingletonManager {
  ...
  private object Internal {
    ...
    case class LeaderData(singleton: ActorRef, singletonTerminated: Boolean = false) extends Data
    case class WasLeaderData(singleton: ActorRef, singletonTerminated: Boolean, newLeader: Address) extends Data
     ...
     * request is allowed. Incoming events are buffered and delivered upon
     * `GetNext` request.
     */
...
class ClusterSingletonManager(
  ...
  when(Leader) {
    case Event(LeaderChanged(leaderOption), LeaderData(singleton, singletonTerminated)) ⇒
      leaderChangedReceived = true
      ...
      leaderOption match {
        ...
        case Some(a) ⇒
          ...
          goto(WasLeader) using WasLeaderData(singleton, singletonTerminated, newLeader = a)
        case _ ⇒
          // new leader will initiate the hand over
          stay
      }

    case Event(HandOverToMe, LeaderData(singleton, singletonTerminated)) ⇒
      gotoHandingOver(singleton, singletonTerminated, Some(sender))

    case Event(Terminated(ref), d @ WasLeaderData(singleton, singletonTerminated, newLeader)) if ref == singleton ⇒
  ...
  when(WasLeader) {
    ...
    case Event(HandOverToMe, WasLeaderData(singleton, singletonTerminated, _)) ⇒
      gotoHandingOver(singleton, singletonTerminated, Some(sender))

    case Event(MemberDowned(m), WasLeaderData(singleton, singletonTerminated, newLeader)) if m.address == newLeader ⇒
      addDowned(m.address)
      gotoHandingOver(singleton, singletonTerminated, None)
  ...
  def gotoHandingOver(singleton: ActorRef, singletonTerminated: Boolean, handOverTo: Option[ActorRef]): State = {
    if (singletonTerminated) {
      handOverDone(handOverTo, handOverData = None)
    } else {
      handOverTo foreach { _ ! HandOverInProgress }
      singleton ! terminationMessage
      goto(HandingOver) using HandingOverData(singleton, handOverTo, None)
    }
  }
  ...
  when(HandingOver) {
    ...
    case Event(singletonHandOverMessage, d @ HandingOverData(singleton, _, None)) if sender == singleton ⇒
